...
Exceptions: correctly and appropriately (without program failure)
            ...
            inncorreclty formatted lead or end descriptors,
...
int getOpCodes(char *fileName, OpCodeType **opCodeDataHead)
   ...
   *opCodeDataHead = NULL;
   ...
   // check first line for correct leader
      // functiion: getLineTo, compareString
   ...
   if(accessResult != COMPLETE_OPCMD_FOUND_MSG)
      ...
      // clear ddata from the structure list
...
Algorithm: usues enum/constant values as indices to select display string,
...
void displayMetaDataError(int code)
   ...
   // display eror to monitor with selected wrror string
      // function: printf
...
Precondition: file is open and file cursor is at beginning
              of an op code
Postcondition: in correct operation,
               finds and returns status as integer
...
int getOpCommand(FILE *filePtr, OpCodeType *inData)
   ...
   // initialize function/variables
      ...
      // initialziw source and destination indices to zero
   ...
   // skip left paren element, incrmeent source index
   sourceIndex++;
   ...
   // check loop overrun failure, check specified lengths
   ...
   // set vlue to data structure component
...
Exceptions: none
Notes: assumes memory access/availablility
*/
...
Algorithm:  ipdates number of "start" op commands found in file
