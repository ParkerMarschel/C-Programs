...
Algorithm: find the length of the string, up to null character
Precondition: given C-style string with null character at the end
Psotcondition: return the number of (int) characters from the beginning 
            up to the null charater
Exceptions: none
...
Precondition: given C-style source string, having null character ('\0')
            at end of string; destination string is passed in as
            parameter with enough memeory to accept the source string
Postcondition: destination string contains exact copy
...
void copyString(char *destination, char *source)
    ...
    // loop until null character is found in the source string
    //  loop kimited to MAX_STR_LEN
...
void concatenateString(char *destination, char *source)
    ...
    while(sourceIndex < MAX_STR_LEN && source[sourceIndex] != NULL_CHAR)
        ...
        // assign null character too next destination element
...
Postcondition: integer value returned as specificied
Exceptions: none
Note: limit test loop to maximum characters for safety
*/
int compareString(char *oneStr, char *otherStr)
    ...
    return getStringLength(oneStr) - getStringLength(otherStr); // temp stub return
    }
...
Precondition: given a C-style source string, having a null character ('\0')
                ...
                with enough memeory to hold the resulting substring 
Postcondition: substring is returned as a parameter
Exceptions: empty string returned if any of the index parameters
            are out of range
Note: copy of source string is made interanally to protect from aliasing
*/
void getSubString(char *destStr, char *sourceStr, int startIndex, int endIndex)
    ...
    if(startIndex >= 0 && startIndex <= endIndex && endIndex < sourceStrLen)
        {
        // create temp string, copoy source string to it
...
Postcondition: index of substring location returned,
                or SUBSTRING_NOT-FOUND constant is returned
Exceptions: none
Note: none
*/
int findSubString(char *testStr, char *searchSubStr)
    ...
    while(masterIndex < testStrLen)
        ...
        while(internalIndex <= testStrLen
            ...
            // check for end of sunstring (searchc completed)
...
Precondition: given a C-style source string, having a null character ('\0')
                ...
                is passed with enough memeory to hold the lower case string
                with enough memeory to hold the resulting substring 
Postcondition: all upper case letters in given string are sert to lower case; no change to any other characters
Exceptions: limit on string loop in case in correct string format
...
Note: none
*/
int getLineTo( FILE *filePtr, int bufferSize, char stopChar, char *buffer, Boolean omitLeadingWhiteSpace)
    {
    // initialize function/variables
        ...
        // initialize status return NO_ERR
        int statusReturn = NO_ERR;

        // initialize buffer size avaliable flag to true
    ...
    // capture string
    // loop while character is not stop character and buffer size is avaliable
    while(charAsInt != (int) stopChar && bufferSizeAvailable == True)
        {
        // check for input faliure
